...
function createGlobalSync() {
  ...
  const sdkFor = (directory: string) => {
    ...
    const sdk = createOpencodeClient({
      ...
      fetch: globalSDK.fetch as any, // Use the authFetch from globalSDK
      directory,
      throwOnError: true,
    })
  ...
  async function loadSessions(directory: string) {
    ...
    const promise = loadRootSessionsWithFallback({
      ...
      .catch((err) => {
        ...
        showToast({ title: "Failed to load sessions", description: err.message })
      })
  ...
  const unsub = globalSDK.event.listen((e) => {
    ...
    applyDirectoryEvent({
      ...
      loadLsp: () => {
        // LSP status not available in opencode-ref SDK for now
      },
    })
  })
  ...
  async function bootstrap() {
    await bootstrapGlobal({
      ...
      connectErrorDescription: `Could not connect to server at ${globalSDK.url}`,
...
export function GlobalSyncProvider(props: ParentProps) {
  ...
  // We provide the context and render children immediately to avoid blank screen.
  // The 'ready' state can be handled individually by components if needed.
  return (
