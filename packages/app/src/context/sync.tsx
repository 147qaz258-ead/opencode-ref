    ...
    return {
      ...
      session: {
        get(sessionID: string) {
          ...
          // First try to find in current directory's sessions
          const match = Binary.search(store.session, sessionID, (s) => s.id)
          if (match.found) {
            return store.session[match.index]
          }

          // Fallback: search in allSessions (global cache)
          const allSessions = store.allSessions || []

          const globalMatch = Binary.search(allSessions, sessionID, (s: any) => s.id)
          if (globalMatch.found) {
            return allSessions[globalMatch.index]
          }
