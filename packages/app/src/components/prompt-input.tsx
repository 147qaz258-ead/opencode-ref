...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const handleSubmit = async (event: Event) => {
    ...
    const logRequestError = (err: unknown, operation: string) => {
      const errorObj = err as Partial<{
        status: number
        statusText: string
        data: unknown
        message: string
      ...
      console.error(`[Error] ${operation}:`, errorObj?.message || err)
    }
    ...
    if (!session && isNewSession) {
      ...
      if (session) {
        // For sandbox mode (Docker), use simplified URL without directory
        // Check if sessionDirectory is a sandbox path (not a local project)
        const isSandbox = !sessionDirectory || sessionDirectory === "global" || sessionDirectory.startsWith("sandbox:")
        if (isSandbox) {
    ...
    client.session
      .prompt({
        ...
        model,
        messageID,
        parts: requestParts as any,
        variant,
      })
      .catch((err) => {
        // Enhanced error logging
        logRequestError(err, "Send Prompt")
        ...
        let userMessage = errorMessage(err)
        if (errorObj.status === 400) {
          if (errorObj.data && typeof errorObj.data === "object") {
            const data = errorObj.data as { error?: string }
            userMessage = data.error || "请求格式错误。请检查输入内容。"
          } else {
            userMessage = "发送失败：请求格式错误。可能是数据验证问题。"
          }
        } else if (errorObj.status === 404) {
          userMessage = "发送失败：会话不存在或已被删除。"
        } else if (errorObj.status === 403) {
          userMessage = "发送失败：没有权限执行此操作。"
        } else if (errorObj.status === 500) {
          userMessage = "发送失败：服务器内部错误。请稍后重试。"
        }
  ...
  return (
    <div class="relative size-full _max-h-[320px] flex flex-col gap-3">
      ...
      >
        <Show when={props.mountedSkill}>
          <div class="px-3 pt-3 pb-0">
            <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-surface-highlight border border-border-weak rounded-full text-sm">
              <span class="text-text-strong">{props.mountedSkill}</span>
              <button
                type="button"
                onClick={(e) => {
                  e.preventDefault()
                ...
                class="text-text-weak hover:text-text-strong flex items-center justify-center"
              >
        ...
        <div class="relative p-3 flex items-center justify-between">
              ...
              <Match when={store.mode === "normal"}>
                <TooltipKeybind placement="top" title="Cycle agent" keybind={command.keybind("agent.cycle")}>
                  <Select
                    ...
                    onSelect={(value: string | undefined) => local.agent.set(value)}
                    class="capitalize"
                    variant="ghost"
                  />
                </TooltipKeybind>
          ...
          <div class="flex items-center gap-3 absolute right-2 bottom-2">
            ...
            >
              <Show
                when={props.submitLabel}
                fallback={
                  <IconButton
                    type="submit"
                    data-testid="send-button"
                    disabled={!prompt.dirty() && !working()}
                    icon={working() ? "stop" : ((props.submitIcon as any) ?? "arrow-up")}
                    variant="primary"
                    class="h-6 w-4.5"
                  />
                }
              >
                ...
                >
                  <Show when={working()} fallback={props.submitLabel}>
                    Stop
                  </Show>
                  <Icon
                    name={working() ? "stop" : ((props.submitIcon as any) ?? "arrow-right")}
                    size="small"
                    class="ml-1.5"
                  />
                </Button>
