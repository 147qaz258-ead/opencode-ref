...
export function SessionHeader() {
  ...
  function navigateToSession(session: Session | undefined) {
    ...
    const directory = session.directory || projectDirectory() || ""
    const encoded = directory === "global" ? "global" : base64Encode(directory)
    navigate(`/${encoded}/session/${session.id}`)
  }

  return (
    <header data-component="session-header" class="h-12 shrink-0 bg-background-base border-b border-border-weak-base flex">
      <button
        type="button"
        ...
        <div class="flex items-center gap-3 min-w-0">
          ...
          <Show when={currentSession() && !parentSession()}>
            <TooltipKeybind class="hidden xl:block" title="New session" keybind={command.keybind("session.new")}>
              <IconButton as={A} href={`/${params.dir}/session`} data-testid="create-session-button" icon="edit-small-2" variant="ghost" />
            </TooltipKeybind>
          </Show>
        </div>
          ...
          <div class="flex items-center gap-1">
              ...
              >
                ...
                >
                  <div class="relative flex items-center justify-center size-4 [&>*]:absolute [&>*]:inset-0">
                    <Icon name="pencil-line" size="small" class="group-hover/review-toggle:hidden" />
                  </div>
                </Button>
              </TooltipKeybind>
            </Show>
            <Show when={currentSession()}>
              <TooltipKeybind
                ...
                keybind={command.keybind("artifacts.open")}
              >
                <Button
                  variant="ghost"
                  class="group/artifacts-toggle size-6 p-0"
                  onClick={() => {
                    command.trigger("artifacts.open")
                  }}
                >
                  <div class="relative flex items-center justify-center size-4">
                    <Icon name="archive" size="small" class="group-hover/artifacts-toggle:text-icon-brand-base" />
                  </div>
                </Button>
              </TooltipKeybind>
            </Show>
