...
export namespace LLM {
  ...
  export async function stream(input: StreamInput) {
    ...
    console.log(`[LLM] Starting stream for session ${input.sessionID}, model: ${input.model.id}`)

    try {
      const [language, cfg] = await Promise.all([Provider.getLanguage(input.model), Config.get()])
      console.log(`[LLM] Got language model and config`)
      ...
      l.error("stream failed", { error: error instanceof Error ? error.message : String(error) })
      throw error
    }
  }
