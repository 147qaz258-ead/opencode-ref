...
export namespace Permission {
  ...
  export function respond(input: { sessionID: Info["sessionID"]; permissionID: Info["id"]; response: Response }) {
    ...
    const sessionPending = pending[input.sessionID]
    if (sessionPending) {
      delete sessionPending[input.permissionID]
    }
    ...
    if (input.response === "reject") {
      const callID = match.info.callID
      match.reject(new RejectedError(input.sessionID, input.permissionID, callID, match.info.metadata))
      return
    }
    match.resolve()
    if (input.response === "always") {
      approved[input.sessionID] = approved[input.sessionID] || {}
      const pattern = match.info.pattern
      const type = match.info.type
      if (pattern && type) {
        const approveKeys = toKeys(pattern, type)
      ...
      if (!items) return
      for (const item of Object.values(items)) {
        const itemPattern = item.info.pattern
        const itemType = item.info.type
        if (itemPattern && itemType) {
          const itemKeys = toKeys(itemPattern, itemType)
