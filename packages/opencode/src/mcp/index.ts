...
export namespace MCP {
  ...
  async function create(key: string, mcp: Config.Mcp) {
    ...
    if (mcp.type === "local") {
      // Local MCP servers are not supported in sandbox mode
      const errorMsg = `Local MCP servers are not supported in sandbox mode.

The MCP server "${key}" is configured as type: "local", which runs commands on the host machine.
However, OpenCode is now running in sandbox-only mode where all operations are contained
within the Docker sandbox environment.

To use MCP with sandbox mode, please:

1. Use remote MCP servers (type: "remote") with HTTP/SSE transport:
   mcp:
     ${key}:
       type: remote
       url: https://your-mcp-server.com/sse

2. Or host your MCP server as an HTTP service accessible from the sandbox

3. Disable this MCP server if not needed:
   mcp:
     ${key}:
       enabled: false

See MCP configuration documentation for more details.`

      log.error("local mcp not supported in sandbox mode", { key, command: mcp.command })
      return {
        mcpClient: undefined,
        status: {
          status: "failed" as const,
